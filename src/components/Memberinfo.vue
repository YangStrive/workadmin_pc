<template>
  <div class="memberinfo">
    ...
    <!-- 弹窗————签约记录 -->
    <div class="dialog-addmember">
      <el-dialog title="签约记录" :visible.sync="dialogContractRecord" size="large">
        <div class="contract_record">
          <el-table
          :data="contract_record"
          tooltip-effect="dark"
          ...
          >
            ...
            >
              <template slot-scope="scope">
                <el-tag
                  type="gray"
                  v-if="scope.row.protocol_status == 0 || scope.row.protocol_status == -1 || scope.row.protocol_status == 3"
                >签订中</el-tag>
                <el-tag
                  type="success"
                ...
                <el-tag
                  type="warning"
                  v-if="scope.row.protocol_status == 2"
                >已撤回</el-tag>
              </template>
            </el-table-column>
            ...
            >
              <template slot-scope="scope">
                ...
                <span class="icon2" v-if="scope.row.opt == 1" @click="seeProtocol(scope.row)">查看</span>
                <span class="icon3" v-if="scope.row.opt == 1" @click="downloadProtocol(scope.row)">下载</span>
                </div>
              </template>
            </el-table-column>
      ...
      <el-dialog title="签署合同" :visible.sync="dialogProtocol" @close="resetForm('ruleForm2')">
        <div class="myform">
          <el-form :model="ruleForm2" :rules="rules2" ref="ruleForm2" label-width="90px" class="demo-ruleForm">
            ...
            <el-form-item label="试用期" prop="probation" v-if="contract_mode != 2">
    ...
    <!-- 上传合同 -->
    <div class="dialog-addmember">
      <el-dialog title="上传合同" :visible.sync="dialogUploadProtocol">
        <div class="dialog_main">
          <el-form :model="uploadProtocolData" label-width="120px">
            <el-form-item label="上传附件">
              <elUpload  :width="255" :limitCount="1" ref="uploadRef" :elTips="protocolTips" @getNewFileListFun="getNewFileListFun" :fileListValue="contractFile"></elUpload>
            </el-form-item>

          </el-form>
        </div>
        <div slot="footer" class="dialog-footer">
          <el-button @click="cancelUploadProtocol">取 消</el-button>
          <el-button type="primary" @click="submitUploadProtocol">确 定</el-button>
        </div>
      </el-dialog>
    </div>
    <!-- 打印信用报告 -->
...
export default {
  ...
  data() {
    ...
    return {
      ...
      dialogUploadProtocol: false, //上传合同弹窗
  ...
  methods: {
    ...
    getContractRecord(){
      util.ajax({
        ...
        success: (obj) => {
          if (obj && obj.errno == 0) {
            this.contract_record = obj.data;
            console.log("contract_record:" + obj.data);
          }
        },
        error: (xhr, status) => {
        },
    ...
    uploadProtocol(){
      ...
      util.ajax({
        ...
        success: (obj) => {
          ...
          this.loading = false;
          if (obj && obj.errno == 0) {
            this.dialogUploadProtocol = true;
          } else {
            this.$message({
              showClose: true,
              message: obj.errmsg,
    ...
    cancelUploadProtocol(){
      this.dialogUploadProtocol= false;
      this.contractFile3=[];
    },
    //上传合同
    submitUploadProtocol() {
      this.loading = true;
      const resignationDocument = {
        file: this.contractFile.length > 0 ? this.contractFile[0].url : null
      };
      console.log(resignationDocument);
      util.ajax({
        url: "/protocol/updateV2",
        type: "POST",
        data: {
          team_id: this.team_id,
          project_id: this.project_id,
          to_user_id: this.member_user_id,
          snapshot_url:resignationDocument.file,
        },
        timeout: 10000,
        success: (obj) => {
          if (obj && obj.errno == 0) {
    ...
    contractRecord() {
      // if (!this.stationList.length) {
          this.dialogContractRecord = true;
      // } else {
        // this.dialogContractRecord = true;
      // }
    },
...
.contract_right > span.icon2 {
  background: url(../assets/imgs/see.svg) left center no-repeat;
}

.contract_right > span.icon3 {
  text-indent: 22px;
  background: url(../assets/imgs/xiazai.svg) 1px center no-repeat;
}
